...
def generate_force_gates_action(environment, current_state, version=3):
    ...
    if len(gates) == 1 and version == 2:
        raise NotImplementedError('Function not implemented, see official source code: '
                                  'generate_sparse_state_action(environment, gates[0])')
    ...
    available_edges = set(filter(lambda e: e[0] not in protected_nodes and
                                           e[1] not in protected_nodes, environment.edge_list))
    ...
    force_mask = [1 if val == 1 else -1 if environment.edge_list[i] not in
                                           available_edges else 0 for i, val in enumerate(action)]
